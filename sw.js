const CACHE_NAME = 'artikel-trainer-v1';
...
self.addEventListener('push', e => {
  let data = { title: 'Artikel Trainer', body: 'Zeit zum Üben! / Time to practice!', icon: '/icons/icon-192.png' };
...
self.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(cs => {
      ...
      else return clients.openWindow('/?screen=practice');
    })
  );
});
...
async function checkAndNotify() {
  ...
  const icon = hour >= 22 ? '/icons/icon-red-192.png' : hour >= 19 ? '/icons/icon-yellow-192.png' : '/icons/icon-192.png';
  const body = hour >= 22
  ...
  await self.registration.showNotification('Artikel Trainer', { body, icon, badge: '/icons/icon-192.png', tag: 'daily-reminder', renotify: true });
}
  ...
  if (e.data.type === 'SCHEDULE_NOTIFICATION') {
    for (const r of (e.data.reminders || [])) {
      ...
      if (delay > 0 && delay < 86400000) {
        setTimeout(() => {
          const icon = r.type === 'lastcall' ? '/icons/icon-red-192.png' : r.type === 'evening' ? '/icons/icon-yellow-192.png' : '/icons/icon-192.png';
